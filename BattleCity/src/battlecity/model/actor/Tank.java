...
import battlecity.model.TankSpecification;
import battlecity.model.Vvram;
...
public abstract class Tank extends Actor {
    ...
    /** ダメージ。ダメージがヒットポイントと同じになったら死亡。コンピュータ戦車の1種類を除き、ヒットポイントは1 */
    private int damage;
    ...
    /** 仮想VRAM */
    protected Vvram vvram; // FIXME:これは各戦車の保持するプロパティではない

    /** 他の戦車のリスト */
    protected Tank[] tanks; // FIXME:これは各戦車の保持するプロパティではない

    /***/
    private TankSpecification tankSpecification;
     ...
     */
    public final TankSpecification getTankSpecification() {
	return tankSpecification;
    }
     ...
     * @param dir
     *            int
     * @param tankSpecification
     *            TankSpecification
     */
    public void spawn(int x, int y, int dir, TankSpecification tankSpecification) {
	...
	this.tankSpecification = tankSpecification;
     ...
     */
    public final boolean move(int newDir) {
	...
	if ((newDir != -1) && (this.direction != newDir)) {
	    ...
	    for (i = tankSpecification.getGunSpeed(); i > 0; i--) {
		moved |= moveSub();
		if (!moved)
		    break;
	    }
	...
	case 1: // 右
	{
	    ...
	    newY = getY();
	    int x = (getX() + 32) / 16;
	    newPositionInVVRAM[0].x = x;
	    newPositionInVVRAM[1].x = x;
	...
	case 2: // 上
	{
	    ...
	    newPositionInVVRAM[1].x = (getX() + 31) / 16;
	    int y = (getY() - 1) / 16;
	    newPositionInVVRAM[0].y = y;
	    newPositionInVVRAM[1].y = y;
	    break;
	}
	default: // case 3: // 左
	{
     ...
     */
    final boolean addDamage() {
	damage++;
	boolean alive = damage < getTankSpecification().getHitPoint();
	return alive;
    }
	...
	case SPAWNING:
	    ...
	    break;
	case ALIVE:
	    if (damage >= getTankSpecification().getHitPoint()) {
		// 死んでしまった
