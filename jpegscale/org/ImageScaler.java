...
public class ImageScaler {
	static BufferedImage scale(BufferedImage src, // ソースイメージ
		...
		for (dy = 0, gy = 0; dy < dest_height; dy++, gy += src_height) {
			...
			for (dx = 0, gx = 0; dx < dest_width; dx++, gx += src_width) {
				...
				// 上辺部分の重みつき画素値を計算
				wrgb.add(calc_row(sx1, sw1, dest_width, sx2, sw2, sy1, sh1, src));
				...
				// 下辺部分の重みつき画素値を計算
				if (sy2 > sy1) {
					wrgb.add(calc_row(sx1, sw1, dest_width, sx2, sw2, sy2, sh2,
							src));
				}
	...
	// 1ライン分の重み付き画素値を計算する
	static WRGB calc_row(int sx1, int sw1, int width, int sx2, int sw2, int y,
			int height, BufferedImage src) {
		WRGB wrgb = new WRGB();
