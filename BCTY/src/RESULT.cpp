...
static void drawPlayerNantoka(int n, int score, int player_x, int score_x, int points1_x, int arrow_x, int arrow_char) {
	bf_mvcprintf(player_x, PLAYER_Y, PLAYER_COLOR1, PLAYER_COLOR2,
		"%d-PLAYER", n + 1);
	bf_mvcprintf(score_x, SCORE_Y, SCORE_COLOR1, SCORE_COLOR2, "%05d00",
		score);
	for (int j = 0; j < 4; j++) {
		bf_mvcprintf(points1_x, POINTS_Y + 48 * j, POINTS_COLOR1,
			POINTS_COLOR2, "%d00pts", j + 1);
		bf_mvcputc(arrow_x, ARROW_Y + 48 * j, ARROW_COLOR1,
			ARROW_COLOR2, arrow_char);
	}
}

void drawPlayerNantoka2(int points2_x, int points3_x, int enemy_tank_index, int enemy_tank_count, int point) {
	bf_mvprintf(points2_x, POINTS_Y + 48 * enemy_tank_index, "%2d", enemy_tank_count);
	bf_mvprintf(points3_x, POINTS_Y + 48 * enemy_tank_index, "%4d", point);
}

// ステージを終了したあと結果を表示し、ボーナスの判定もする
// mode bit 0,1 でプレイヤー1,2のスコアを計算するかを表す
//void result(int mode, int score[2], Hits hits[]) {
void GameView::showStageResultScreen() {
	int score[2];
	// model->getHighScore(
	int total[2] = {0, 0};
	...
	bf_printf("%05d00", model->getHighScore());
	...
	drawPlayerNantoka(1, score[0], player_x[0], score_x[0], points1_x[0], arrow_x[0], arrow_char[0]);
	if (this->model->is2pMode()) {
		drawPlayerNantoka(2, score[1], player_x[1], score_x[1], points1_x[1], arrow_x[1], arrow_char[1]);
	}
	...
	// やっつけた戦車の数と得点の表示
	{
		int i;
	for (i = 0; i < 4; i++) {
//		total[j] += hits[0].num[i];
//		p = hits[0].num[i] * (i + 1);
//		const PlayerInformation *pi = this->model->getPlayerInformation(1);
//		drawPlayerNantoka2(points2_x[0], points3_x[0], i, pi->);
		if (model->is2pMode()) {
//			drawPlayerNantoka2(i);
		}
#if !defined(PROF)
	...
	bf_mvprintf(points2_x[0], TOTAL_Y + 16, "%2d", total[0]);
	if (model->is2pMode())
		bf_mvprintf(points2_x[1], TOTAL_Y + 16, "%2d", total[1]);
#if !defined(PROF)
	sound_out(EFS_RESULT);
#endif
	wait_button();
