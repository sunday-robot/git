...
#include "tr_types.h"
#include "../cgame_game_q3_ui/bg_public.h"
#include "cg_public.h"
...
typedef struct {
	...
	// (おそらく)画面にオーバーレイ表示される各種文字列(メッセージ、メニュー項目等)の座標は、640x480の画面でのものが定義されている。
	// これら座標値を、実際の画面サイズ(1366x768とか1920x1080等)に合わせるための係数(元の座標値にこの係数を乗じる)。
	float			screenXScale;		// derived from glconfig
	float			screenYScale;

	// 上のscreenXScaleと同様に、実際の画面サイズに合うように座標値を調整するための係数だが、ほとんど使われておらず、そもそも値がセットされない(おそらく初期値の0)のまま。
	float			screenXBias;
