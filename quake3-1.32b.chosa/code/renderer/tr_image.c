...
static void R_CreateDlightImage(void) {
	...
	tr.dlightImage = R_CreateImage("*dlight", (byte *) data, DLIGHT_SIZE, DLIGHT_SIZE, 0, 0, GL_CLAMP);
}
...
static void R_CreateFogImage(void) {
	...
	// what we want.
	tr.fogImage = R_CreateImage("*fog", (byte *) data, FOG_S, FOG_T, 0, 0, GL_CLAMP);
...
static void R_CreateDefaultImage(void) {
	...
	tr.defaultImage = R_CreateImage("*default", (byte *) data, DEFAULT_SIZE, DEFAULT_SIZE, 1, 0, GL_REPEAT);
}
...
void R_CreateBuiltinImages(void) {
	...
	tr.whiteImage = R_CreateImage("*white", (byte *) data, 8, 8, 0, 0, GL_REPEAT);
	...
	tr.identityLightImage = R_CreateImage("*identityLight", (byte *) data, 8, 8, 0, 0, GL_REPEAT);


	for (x = 0; x < 32; x++) {
		// scratchimage is usually used for cinematic drawing
		tr.scratchImage[x] = R_CreateImage("*scratch", (byte *) data, DEFAULT_SIZE, DEFAULT_SIZE, 0, 1, GL_CLAMP);
	}
...
qhandle_t RE_RegisterSkin(const char *name) {
	...
	if (strcmp(name + strlen(name) - 5, ".skin")) {
		...
		skin->surfaces[0]->shader = R_FindShader(name, LIGHTMAP_NONE, 1);
		return hSkin;
	}
	...
	while (text_p && *text_p) {
		...
		surf->shader = R_FindShader(token, LIGHTMAP_NONE, 1);
		skin->numSurfaces++;
	}
