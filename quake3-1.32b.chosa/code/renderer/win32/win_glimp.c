...
static qboolean s_classRegistered = 0;
										  ...
										  qboolean cdsFullscreen)
{
	...
	// TODO AKIYAMA フルスクリーンモードにすると、画面の状態がおかしくなる。
#if 1
	err = GLW_SetMode(drivername, r_mode->integer, colorbits, cdsFullscreen);
#else
	err = GLW_SetMode(drivername, r_screenWidth->integer, r_screenHeight->integer, colorbits, cdsFullscreen);
#endif
...
#endif
{
	...
	if (cdsFullscreen) {
		...
		if (glw_state.cdsFullscreen) {
			...
			if (!GLW_CreateWindow(drivername, glConfig.vidWidth, glConfig.vidHeight, colorbits, 1)) {
		...
		//
			...
			if ((cdsRet = ChangeDisplaySettings(&dm, CDS_FULLSCREEN)) == DISP_CHANGE_SUCCESSFUL) {
				...
				if (modeNum != -1 && (cdsRet = ChangeDisplaySettings(&devmode, CDS_FULLSCREEN)) == DISP_CHANGE_SUCCESSFUL) {
					...
					glw_state.cdsFullscreen = 0;
					glConfig.isFullscreen = 0;
					if (!GLW_CreateWindow(drivername, glConfig.vidWidth, glConfig.vidHeight, colorbits, 0)) {
						return RSERR_INVALID_MODE;
					}
...
static void GLW_InitExtensions(void) {
	...
	if (qwglSwapIntervalEXT) {
		...
		r_swapInterval->modified = 1;	// force a set next frame
	} else {
...
#else
		if (!GLW_StartDriverAndSetMode(drivername, r_screenWidth->integer, r_screenHeight->integer, r_colorbits->integer, cdsFullscreen))
#endif
		{
			...
			if (glConfig.driverType == GLDRV_ICD) {
				...
				if (r_colorbits->integer != 16 ||
					cdsFullscreen != 1 ||
					r_mode->integer != 3)	// r_mode==3は、640*480
#else
				if (r_colorbits->integer != 16 ||
					cdsFullscreen != 1 ||
					r_screenWidth->integer != 640 ||
					r_screenHeight->integer != 480)
#endif
				{
#if 1
					if (!GLW_StartDriverAndSetMode(drivername, 3, 16, 1))
#else
#endif
					{
						goto fail;
...
void GLimp_EndFrame(void) {
	...
	// check logging
	QGL_EnableLogging(r_logFile->integer ? 1 : 0);
}
