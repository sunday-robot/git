...
static void ParseSkyParms( char **text ) {
	...
	if ( strcmp( token, "-" ) ) {
		for (i=0 ; i<6 ; i++) {
			Com_sprintf( pathname, sizeof(pathname), "%s_%s.tga"
				, token, suf[i] );
			shader.sky.outerbox[i] = R_FindImageFile( ( char * ) pathname, 1, 1, GL_CLAMP );
	...
	if ( strcmp( token, "-" ) ) {
		for (i=0 ; i<6 ; i++) {
			Com_sprintf( pathname, sizeof(pathname), "%s_%s.tga"
				, token, suf[i] );
			shader.sky.innerbox[i] = R_FindImageFile( ( char * ) pathname, 1, 1, GL_REPEAT );
...
shader_t *R_FindShader( const char *name, int lightmapIndex, qboolean mipRawImage ) {
	...
	if ( shader.lightmapIndex == LIGHTMAP_NONE ) {
		...
		stages[0].bundle[0].isLightmap = 1;
		stages[0].active = 1;
...
qhandle_t RE_RegisterShaderLightMap( const char *name, int lightmapIndex ) {
	...
	sh = R_FindShader( name, lightmapIndex, 1 );
...
qhandle_t RE_RegisterShader( const char *name ) {
	...
	sh = R_FindShader( name, LIGHTMAP_2D, 1 );
...
qhandle_t RE_RegisterShaderNoMip( const char *name ) {
	...
	sh = R_FindShader( name, LIGHTMAP_2D, 0 );
...
static void CreateExternalShaders( void ) {
	tr.projectionShadowShader = R_FindShader( "projectionShadow", LIGHTMAP_NONE, 1 );
	tr.flareShader = R_FindShader( "flareShader", LIGHTMAP_NONE, 1 );
	tr.sunShader = R_FindShader( "sun", LIGHTMAP_NONE, 1 );
}
