...
vidmode_t r_vidModes[] =
{
	...
	{"Mode 13: 1366x768", 1366, 768, 1},
	{"Mode 14: 960x540", 960, 540, 1},
#endif
};
...
/*
*/
static void takeScreenshot(int x, int y, int width, int height, char *fileName) {
	byte		*buffer;
	int			i, c, temp;
...
/*
*/
void takeScreenshotJPEG(int x, int y, int width, int height, char *fileName) {
	byte		*buffer;
...
/*
*/
static void takeScreenshotR(int x, int y, int width, int height, char *name, qboolean jpeg) {
...
/*
*/
void screenshotFilename(int lastNumber, char *fileName) {
	int		a, b, c, d;
...
==================
*/
static void screenshotFilenameJPEG(int lastNumber, char *fileName) {
	int		a, b, c, d;
...
void screenShot_f(void) {
	...
	takeScreenshotR(0, 0, glConfig.vidWidth, glConfig.vidHeight, checkname, 0);
...
static void screenShotJPEG_f(void) {
	...
	takeScreenshotR(0, 0, glConfig.vidWidth, glConfig.vidHeight, checkname, 1);
...
void registerValues(void) {
	...
	assertCvarRange(r_picmip, 0, 16, 1);
...
#else
	r_screenWidth = ri.Cvar_Get("r_screenWidth", "640", CVAR_ARCHIVE | CVAR_LATCH);
	r_screenHeight = ri.Cvar_Get("r_screenHeight", "480", CVAR_ARCHIVE | CVAR_LATCH);
#endif
	...
	assertCvarRange(r_displayRefresh, 0, 200, 1);
	...
	assertCvarRange(r_znear, 0.001f, 200, 1);
	...
	// 画面解像度については自由に設定できるようにしてみる。
#if 1
	ri.Cmd_AddCommand("modelist", modeList_f);
#else
#endif
	ri.Cmd_AddCommand("screenshot", screenShot_f);
	ri.Cmd_AddCommand("screenshotJPEG", screenShotJPEG_f);
