...
//		Com_Memcpy(font, faceData, sizeof(fontInfo_t));
  ...
  for (i = GLYPH_START; i < GLYPH_END; i++) {
    glyph = RE_ConstructGlyphInfo(out, &xOut, &yOut, &maxHeight, face, (unsigned char)i, 1);
  }
  ...
  while ( i <= GLYPH_END ) {
    ...
    glyph = RE_ConstructGlyphInfo(out, &xOut, &yOut, &maxHeight, face, (unsigned char)i, 0);
    ...
    if (xOut == -1 || yOut == -1 || i == GLYPH_END)  {
      ...
      image = R_CreateImage(name, imageBuff, 256, 256, 0, 0, GL_CLAMP);
      h = RE_RegisterShaderFromImage(name, LIGHTMAP_2D, image, 0);
