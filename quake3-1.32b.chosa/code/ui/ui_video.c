...
#if 0
static char **getResolutionList() {
	static char **resolutionList = NULL;
	...
	for (int i = 0; i < 100; i++) {
		...
		int h;
		float ratio;
		if (!R_GetModeInfo(&w, &h, &ratio, i)) {
			resolutionList[i] = NULL;
			break;
		}
		int len = sprintf(p, "%dx%d", w, h);
		p += len + 1;
	}
...
#else
	s_graphicsoptions.mode.itemnames = getResolutionList();
#endif
