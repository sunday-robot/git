    ...
    LPARAM  lParam)
{
	...
	if (in_mouse->integer != 1)
	{
		if ( uMsg == MSH_MOUSEWHEEL )
		{
			if ( ( ( int ) wParam ) > 0 )
			{
				Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, K_MWHEELUP, 1, 0, NULL );
				Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, K_MWHEELUP, 0, 0, NULL );
			}
			else
			{
				Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, K_MWHEELDOWN, 1, 0, NULL );
				Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, K_MWHEELDOWN, 0, 0, NULL );
			}
	...
	case WM_MOUSEWHEEL:
		...
		if (in_mouse->integer != 1 || (!r_fullscreen->integer && (cls.keyCatchers & KEYCATCH_CONSOLE)))
		{
			...
			if ( zDelta > 0 )
				...
				zDelta = -zDelta;
				up_down = K_MWHEELDOWN;
			}
			for(i=0; i<zDelta; i++) {
				if (!in_logitechbug->integer) {
					Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, up_down, 1, 0, NULL );
					Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, up_down, 0, 0, NULL );
				} else {
					Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, up_down, flip, 0, NULL );
					flip = !flip;
				}
			}
...
// one update sometimes, so we always check all states and look for events
	...
	case WM_SYSKEYDOWN:
		...
		// fall through
	case WM_KEYDOWN:
		Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, MapKey( lParam ), 1, 0, NULL );
		break;

	case WM_SYSKEYUP:
	case WM_KEYUP:
		Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, MapKey( lParam ), 0, 0, NULL );
		break;
