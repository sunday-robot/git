...
void Sys_CreateConsole(void) {
	...
	int swidth = GetDeviceCaps(hDC, HORZRES);	// ディスプレイの幅[pixel](コンソールウィンドウの表示位置を計算するためのもの)
	int sheight = GetDeviceCaps(hDC, VERTRES);	// ディスプレイの高さ[pixel](コンソールウィンドウの表示位置を計算するためのもの)
