...
gotnewcl:
	...
	*newcl = temp;
	...
	denied = (char *) VM_Call(gvm, GAME_CLIENT_CONNECT, clientNum, 1, 0); // firstTime = 1
	if (denied) {
...
void SV_SendClientGameState(client_t *client) {
	...
	for (start = 0; start < MAX_GENTITIES; start++) {
		...
		MSG_WriteDeltaEntity(&msg, &nullstate, base, 1);
	}
...
void SV_WriteDownloadToClient(client_t *cl, msg_t *msg) {
	...
	if (!cl->download) {
		...
		missionPack = FS_IsIdPak(cl->downloadName, "missionpack");
		idPack = missionPack || FS_IsIdPak(cl->downloadName, "baseq3");
...
void SV_ExecuteClientMessage(client_t *cl, msg_t *msg) {
	...
	// read the usercmd_t
	if (c == clc_move) {
		SV_UserMove(cl, msg, 1);
	} else if (c == clc_moveNoDelta) {
		SV_UserMove(cl, msg, 0);
	} else if (c != clc_EOF) {
