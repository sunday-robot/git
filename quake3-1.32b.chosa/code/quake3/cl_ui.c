...
*/
static int cl_UISystemCalls(int *args) {
	switch (args[0]) {
	case UI_ERROR:
...
qboolean UI_usesUniqueCDKey() {
	if (uivm) {
		return (VM_Call(uivm, UI_HASUNIQUECDKEY) == 1);
	} else {
		return 0;
	}
}
...
qboolean UI_GameCommand(void) {
	...
	return VM_Call(uivm, UI_CONSOLE_COMMAND, cls.realtime) ? 1 : 0;
}
