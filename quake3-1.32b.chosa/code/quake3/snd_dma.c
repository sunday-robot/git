...
void S_BeginRegistration(void) {
	...
	if (s_numSfx == 0) {
		...
		S_RegisterSound("sound/feedback/hit.wav", 0);		// changed to a sound in baseq3
	}
}
...
void S_StopLoopingSound(int entityNum) {
	loopSounds[entityNum].active = 0;
	//	loopSounds[entityNum].sfx = 0;
	loopSounds[entityNum].kill = 0;
}
...
void S_ClearLoopingSounds(qboolean killall) {
	...
	for (i = 0; i < MAX_GENTITIES; i++) {
		if (killall || loopSounds[i].kill == 1 || (loopSounds[i].sfx && loopSounds[i].sfx->soundLength == 0)) {
			loopSounds[i].kill = 0;
...
void S_AddLoopingSound(int entityNum, const vec3_t origin, const vec3_t velocity, sfxHandle_t sfxHandle) {
	...
	loopSounds[entityNum].active = 1;
	loopSounds[entityNum].kill = 1;
	loopSounds[entityNum].doppler = 0;
	...
	if (s_doppler->integer && VectorLengthSquared(velocity) > 0.0) {
		...
		loopSounds[entityNum].doppler = 1;
		...
		if (loopSounds[entityNum].dopplerScale <= 1.0) {
			loopSounds[entityNum].doppler = 0;			// don't bother doing the math
		}
	}
...
Returns 1 if any new sounds were started since the last mix
========================
*/
...
void S_StartBackgroundTrack(const char *intro, const char *loop) {
	...
	//
	FS_FOpenFileRead(name, &s_backgroundFile, 1);
	if (!s_backgroundFile) {
