...
void CL_Record_f(void) {
	...
	for (i = 0; i < MAX_GENTITIES; i++) {
		...
		MSG_WriteDeltaEntity(&buf, &nullstate, ent, 1);
	}
...
/*
*/
static void cl_WalkDemoExt(
	const char *arg,
	char *name,
	int *demofile) {	// [out]デモファイルのハンドル
	*demofile = 0;
	for (int i = 0; demo_protocols[i] != 0; i++) {
		Com_sprintf(name, MAX_OSPATH, "demos/%s.dm_%d", arg, demo_protocols[i]);
		FS_FOpenFileRead(name, demofile, 1);
		if (*demofile) {
...
この関数で、デモファイルのロードまでは行うが、再生は行わない。
デモファイルの内容を、イベントキューにセットするというだけらしい。
*/
static void cl_PlayDemo_f(void) {
	...
	if ((strlen(arg) > 6) && (ext_test[0] == '.') && ((ext_test[1] == 'd') || (ext_test[1] == 'D')) && ((ext_test[2] == 'm') || (ext_test[2] == 'M')) && (ext_test[3] == '_')) {
		...
		if (demo_protocols[i]) {
			...
			FS_FOpenFileRead(name, &clc.demofile, 1);
		} else {
...
void CL_InitDownloads(void) {
	...
	if (!cl_allowDownload->integer) {
		...
		if (FS_ComparePaks(missingfiles, sizeof(missingfiles), 0)) {
	...
	} else if (FS_ComparePaks(clc.downloadList, sizeof(clc.downloadList), 1)) {
