...
/*
文字列のクローンを生成する。
ただし、空文字列、数字一文字については、すでに用意されている文字列定数を返す。
NOTE:	never write over the memory CopyString returns because
memory from a memstatic_t might be returned
*/
char *CopyString(const char *in) {
	// 空文字列の場合はすでに用意してある空文字列定数を返す。
	if (in[0] == '\0')
		return ((char *) &emptystring) + sizeof(memblock_t);

	// 文字列が1文字の数字の場合は、新たにメモリを確保せずに、あらかじめ各数字用に用意してあるメモリ領域を返す。
...
static void com_InitSmallZoneMemory(void) {
	...
	smallzone = calloc(s_smallZoneTotal, 1);	// malloc()ではダメ(ゼロクリアが必要)らしい
	if (smallzone == NULL) {
...
// com_pushedEvents内で次にイベントを追加すべき場所(つまり最新のイベントの次)を指すインデックス
// (この変数は、プログラム起動時に0に初期化されるだけで、それ以降はインクリメントしかされない。ゲーム終了までにオーバーフローすることはまずあり得ないということらしい)
static int com_pushedEventsHead = 0;

// com_pushedEvents内で一番古い(つまり次に取り出されるべき)イベントを指すインデックス
// (この変数は、プログラム起動時に0に初期化されるだけで、それ以降はインクリメントしかされない。ゲーム終了までにオーバーフローすることはまずあり得ないということらしい)
...
/*
システムのイベントキューからイベント情報を取得し、呼び出し元に返す。
ただし、com_journalが2のときは、イベントキューからではなく、ジャーナルファイルからイベント情報を取得する。
また、com_journalが1のときは、イベントキューから取得したイベント情報をジャーナルファイルへの追加も行う。
*/
static sysEvent_t com_GetRealEvent(void) {
	...
	if (com_journal->integer == 2) {
		// ジャーナルファイルからイベントデータを取得する(デバッグ用?、デモ用?)
		// EOF検知がないけどどうなっているのだろう?終了を示す特別なイベントがあるということか?
		...
		// Windowsなどのイベントキューからイベントデータを取得する(通常はこちらと思う。)
		ev = Sys_GetEvent();

		// write the journal value out if needed
		if (com_journal->integer == 1) {
			// イベントデータをジャーナルファイルに追加する。(リプレイ記録用?)
...
// bk001129 - added
/*
com_pushedEventsの初期化(ゼロクリアしているだけ)
*/
...
/*
Com_Milliseconds()だけから呼び出される。
Com_Milliseconds()は、現在時刻を返すものだが、イベントとの前後関係がおかしくならないため、直接OSから取得したりせず、
イベントキュー内の最古のイベントのタイムスタンプを現在時刻としている。
このタイムスタンプを取り出すためにイベントキューからイベントを取り出してしまうため、本関数で元に戻す。
(もっとスマートな方法がとれるような気がするが、何か事情があると思われる。)
*/
...
/*
イベントキューにイベントがあればそれを返す。
なければ、イベント読み込みを行い、それを返す。
*/
...
/*
イベントキューに溜まっているイベントを処理する。
イベントキューがからの場合、イベント取得を行い、そのイベントを処理する。
...
int Com_EventLoop() {
	...
	for (;;) {
		...
		if (ev.evType == SE_NONE) {
			...
			// ループバックチャンネル(?)に、パケットイベントを手動送信する。
			while (NET_GetLoopPacket(NS_CLIENT, &evFrom, &buf))
				CL_PacketEvent(evFrom, &buf);
			while (NET_GetLoopPacket(NS_SERVER, &evFrom, &buf))
				// if the server just shut down, flush the events
				// サーバーがシャットダウン(中?)の場合、イベントをフラッシュする。
			if (com_sv_running->integer)
				Com_RunAndTimeServerPacket(&evFrom, &buf);
			return ev.evTime;	// ここがこの関数唯一の出口
		}
		...
		default:
			...
			break;
			// SE_NONEの場合は、既にこの関数からreturnしているので、以下の2行は不要。
			//case SE_NONE:
			//	break;
		case SE_KEY:
...
int Com_ModifyMsec(const int in_msec) {
	int clampTime;	// clamp...クランプなどでぎゅっと締める。固定する。

	int msec;
	// modify time for debugging values
	if (com_fixedtime->integer != 0) {	// CPUの負荷などで変動する実時間ではなく、常に一定刻みでゲーム時間を進める
		msec = com_fixedtime->integer;
	} else if (com_timescale->value != 0) {	// 実際よりも遅い/早くゲーム時間を進める
		msec = in_msec * com_timescale->value;
	} else if (com_cameraMode->integer != 0) {	// "camera mode"が何なのかよくわからない。
		msec = in_msec * com_timescale->value;
	} else {
		msec = in_msec;
	}
	...
	if (com_dedicated->integer) {
		...
		// of time.
		// dedicatedサーバーは、全てのクライアント時間のview(?)を台無しにしてしまうので、非常に多くの期間、固定したくない。
		if (msec > 500) {
	...
	if (!com_sv_running->integer) {
		...
		// リモートサーバーのクライアントは、サーバー時間のview(?)を一時的にゆがめてしまうため、時間を固定したくない。
		clampTime = 5000;
	} else {
...
/*
ゲームのトップレベルのメインループから呼び出される、中核となる関数。
時間調整(コンピューターの速度が速すぎる場合の待ちとか…)も、この中で行っている。
*/
void Com_Frame(void) {
	static int lastTime;
	...
	if (com_speeds->integer)
		timeBeforeFirstEvents = Sys_Milliseconds();	// ゲーム起動からの経過時間
	else
		timeBeforeFirstEvents = 0;

	// コンピューターの処理速度が速い場合、ここでイベント処理を行いつつ待つ。
