...
byte MSG_ReadByte(msg_t *msg) {
	byte c = (byte) MSG_ReadBits(msg, 8);
	if (msg->readcount > msg->cursize)
		return 0xff;
...
char *MSG_ReadString(msg_t *msg) {
	...
	l = 0;
	do {
		c = (char) MSG_ReadByte(msg);		// use ReadByte so -1 is out of bounds
		if (c == -1 || c == 0) {
			break;
		}
...
void MSG_ReadDeltaUsercmd(msg_t *msg, usercmd_t *from, usercmd_t *to) {
	...
	to->forwardmove = (char) MSG_ReadDelta(msg, from->forwardmove, 8);
	to->rightmove = (char) MSG_ReadDelta(msg, from->rightmove, 8);
	to->upmove = (char) MSG_ReadDelta(msg, from->upmove, 8);
	to->buttons = MSG_ReadDelta(msg, from->buttons, 16);
	to->weapon = (char) MSG_ReadDelta(msg, from->weapon, 8);
}
...
void MSG_ReadDeltaUsercmdKey(msg_t *msg, int key, usercmd_t *from, usercmd_t *to) {
	...
	if (MSG_ReadBits(msg, 1)) {
		...
		to->forwardmove = (char) MSG_ReadDeltaKey(msg, key, from->forwardmove, 8);
		to->rightmove = (char) MSG_ReadDeltaKey(msg, key, from->rightmove, 8);
		to->upmove = (char) MSG_ReadDeltaKey(msg, key, from->upmove, 8);
