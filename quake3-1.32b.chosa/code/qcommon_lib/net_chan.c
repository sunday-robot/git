...
/*
Returns 0 if the message should not be processed due to being
out of order or a fragment.
メッセージがアウトオフオーダー(?)あるいは断片により処理されるべきではない場合は、0を返す。

Msg must be large enough to hold MAX_MSGLEN, because if this is the
final fragment of a multi-part message, the entire thing will be
copied out.
Msgは、MAX_MSGLENより大きくなければならない。
なぜならこれはマルチパートメッセージの最後の断片である場合、全体はコピーアウトされなければならないからである。
*/
