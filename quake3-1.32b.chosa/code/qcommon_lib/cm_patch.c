...
*/
static qboolean planeFromPoints(vec4_t plane, vec3_t a, vec3_t b, vec3_t c) {
	vec3_t	d1, d2;
...
*/
static qboolean	needsSubdivision(vec3_t a, vec3_t b, vec3_t c) {
	vec3_t		cmid;
	vec3_t		lmid;
	vec3_t		delta;
	float		dist;
...
/*
If the left and right columns are exactly equal, set grid->wrapWidth 1
*/
...
from the true curve
*/
static void subdivideGridColumns(cGrid_t *grid) {
	int		i, j, k;
...
/*
*/
static qboolean comparePoints(float *a, float *b) {
	float		d;
...
static int	gridPlane(int gridPlanes[MAX_GRID_SIZE][MAX_GRID_SIZE][2], int i, int j, int tri) {
	...
	// should never happen
	Com_Printf("WARNING: gridPlane unresolvable\n");
	return -1;
}
...
static int edgePlaneNum(cGrid_t *grid, int gridPlanes[MAX_GRID_SIZE][MAX_GRID_SIZE][2], int i, int j, int k) {
	...
	Com_Error(ERR_DROP, "edgePlaneNum: bad k");
	return -1;
}

/*
*/
static void setBorderInward(facet_t *facet, cGrid_t *grid, int gridPlanes[MAX_GRID_SIZE][MAX_GRID_SIZE][2],
	...
	case 1:
		...
		break;
	default:
		Com_Error(ERR_FATAL, "setBorderInward: bad parameter");
		numPoints = 0;
		break;
	}
	...
	for (k = 0; k < facet->numBorders; k++) {
		...
		if (front && !back) {
			...
			Com_DPrintf("WARNING: setBorderInward: mixed plane sides\n");
			facet->borderInward[k] = 0;
			if (!debugBlock) {
				debugBlock = 1;
...
==================
*/
static qboolean validateFacet(facet_t *facet) {
	float		plane[4];
	int			j;
	winding_t	*w;
	vec3_t		bounds[2];
	...
	if (!w) {
		return 0;		// winding was completely chopped away
	}
	...
	for (j = 0; j < 3; j++) {
		if (bounds[1][j] - bounds[0][j] > MAX_MAP_BOUNDS) {
			return 0;		// we must be missing a plane
		}
...
/*
*/
static void patchCollideFromGrid(cGrid_t *grid, patchCollide_t *pf) {
	int				i, j;
	float			*p1, *p2, *p3;
