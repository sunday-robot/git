...
static void cvar_Set2(const char *var_name, const char *value, qboolean force);
...
// 文字列にcvarの変数名として不正なもの(「\」、「"」、「;」)が含まれていないことを確認する。
...
/*
変数テーブルから変数を探し、返す。無ければNULLを返す。
*/
static cvar_t *findVar(const char *name) {
	cvar_t	*var;
	long hash;
...
すでに変数が存在している場合、CVAR_ROMフラグが立っていない限り、変数はセットされない。
*/
cvar_t *Cvar_Get(const char *name, const char *default_value, int flags) {
	if (!name || !default_value) {
		Com_Error(ERR_FATAL, "Cvar_Get: NULL parameter");
	}

	if (!Cvar_ValidateString(name)) {	// 「\」、「"」、「:」が含まれないことを確認する
		Com_Printf("invalid cvar name string: %s\n", name);
		name = "BADNAME";
	}

	cvar_t *var = findVar(name);
	if (var != NULL) {
		// 生成済みの場合
		...
		if (((var->flags & CVAR_USER_CREATED) != 0)
			...
			Z_Free(var->resetString);	// 現状の「リセット値」(デフォルト値)を開放する。
			var->resetString = CopyString(default_value);
		...
		if (var->resetString[0] != '\0') {
			...
			var->resetString = CopyString(default_value);
		} else if (default_value[0] && strcmp(var->resetString, default_value)) {
			Com_DPrintf("Warning: cvar \"%s\" given initial values: \"%s\" and \"%s\"\n", name, var->resetString, default_value);
		}
	...
	var->string = CopyString(default_value);
	var->modified = 1;
...
static void cvar_Set2(const char *var_name, const char *value, qboolean force) {
	...
	var = findVar(var_name);
	if (var == NULL) {
		// 変数が未定義の場合は以下の処理を行うだけ。
		if (value == NULL)
			return;
		// create it
		if (!force)
			Cvar_Get(var_name, value, CVAR_USER_CREATED);
		else
			Cvar_Get(var_name, value, 0);
		return;
	}

	// 変数が定義済みの場合は、以下の処理を行わなければならない。
...
void Cvar_Set_f(void) {
	...
	for (int i = 2; i < arg_count; i++) {
		int len = strlen(Cmd_Argv(i) + 1);	// +1の意味が分からない。
...
void Cvar_List_f(void) {
	...
	for (var = cvar_vars; var; var = var->next, i++) {
		if (match && !Com_Filter(match, var->name, 0)) continue;
