...
void	CM_FloodAreaConnections (void);	// cm_test.c
...
/*
*/
static void boundBrush( cbrush_t *b ) {
...
static unsigned lumpChecksum(lump_t *lump) {
...
unsigned CM_Checksum(dheader_t *header) {
	...
	checksums[0] = lumpChecksum(&header->lumps[LUMP_SHADERS]);
	checksums[1] = lumpChecksum(&header->lumps[LUMP_LEAFS]);
	checksums[2] = lumpChecksum(&header->lumps[LUMP_LEAFBRUSHES]);
	checksums[3] = lumpChecksum(&header->lumps[LUMP_LEAFSURFACES]);
	checksums[4] = lumpChecksum(&header->lumps[LUMP_PLANES]);
	checksums[5] = lumpChecksum(&header->lumps[LUMP_BRUSHSIDES]);
	checksums[6] = lumpChecksum(&header->lumps[LUMP_BRUSHES]);
	checksums[7] = lumpChecksum(&header->lumps[LUMP_MODELS]);
	checksums[8] = lumpChecksum(&header->lumps[LUMP_NODES]);
	checksums[9] = lumpChecksum(&header->lumps[LUMP_SURFACES]);
	checksums[10] = lumpChecksum(&header->lumps[LUMP_DRAWVERTS]);
