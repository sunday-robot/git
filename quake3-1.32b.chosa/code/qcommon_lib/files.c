...
すべてのQuakeデータアクセスは、階層的なファイルシステム経由するものである。
ただし、ファイルシステムの内容は、いくつかのソールから透過的にマージされる。

A "qpath" is a reference to game file data.  MAX_ZPATH is 256 characters, which must include
a terminating zero. "..", "\\", and ":" are explicitly illegal in qpaths to prevent any
references outside the quake directory system.

"qpath"は、ゲームファイルデータ(?)への参照である。
MAX_ZPATHは、256個のcharで、終端の0を含まなければならない。
".."、"\\"、":"は、qpath群での明示的な違法で、quakeディレクトリシステム外への参照をさせないためである。
...
"base path"は、すべてのゲームディレクトリ、そして通常実行ファイルを保持するディレクトリへのパスである。
それはデフォルトでは"."であるが、"+set fs_basepath c:\quake3"コマンドでオーバーライドできる。
basepathはスタートアップが終わってからは変更できない。
生成されるすべてのファイル(デモ、スクリーンショット等)は、base pathからの相対位置に生成されるので、base pathは書き込み可能でなければならない。
	...
	if (fs_numServerPaks) {
		for (i = 0 ; i < fs_numServerPaks ; i++) {
			...
			if (pack->checksum == fs_serverPaks[i]) {
				return 1;		// on the aproved list
			}
		}
		return 0;	// not on the pure server pak list
	}
	return 1;
}
...
以下の文字列を返す。("\\"はWindows用のみ)
base + "\\" + game + "\\" + qpath

ただし、qpathがNULLの場合は、以下を返す
base + "\\" + game
*/
const char *FS_BuildOSPath(const char *base, const char *game, const char *qpath) {
	...
	FS_ReplaceSeparators(temp);	// セパレータの"/"をOS固有のセパレータ(Windowsなら"\\")に変換するだけのもの。
...
int FS_SV_FOpenFileRead(const char *filename, fileHandle_t *fp) {
  ...
  if (!fsh[f].handleFiles.file.o)
  {
	...
	if (!fsh[f].handleFiles.file.o) {
    // search cd path
    ospath = FS_BuildOSPath("", filename, NULL);
	...
	for (search = fs_searchpaths ; search ; search = search->next) {
		...
		if (search->pack) {
			...
			if (fs_restrict->integer || fs_numServerPaks) {
				...
				sysFiles = Sys_ListFiles(netpath, extension, filter, &numSysFiles, 0);
...
int	FS_GetModList(char *listbuf, int bufsize) {
  ...
  *listbuf = 0;
  ...
  pFiles0 = Sys_ListFiles(fs_homepath->string, NULL, NULL, &dummy, 1);
  pFiles1 = Sys_ListFiles(fs_basepath->string, NULL, NULL, &dummy, 1);
  pFiles2 = Sys_ListFiles("", NULL, NULL, &dummy, 1);
...
void FS_TouchFile_f(void) {
	...
	FS_FOpenFileRead(Cmd_Argv(1), &f, 0);
	if (f) {
		FS_FCloseFile(f);
	}
}
...
static void fs_AddGameDirectory(const char *path, const char *dir) {
	...
	pakfiles = Sys_ListFiles(pakfile, ".pk3", NULL, &numfiles, 0);
...
qboolean FS_IsIdPak(char *pak, char *base) {
	int i;
...
qboolean FS_ComparePaks(char *neededpaks, int len, qboolean dlstring) {
	...
	if (!fs_numServerReferencedPaks) {
		return 0; // Server didn't send any pack information along
	}
	...
	*neededpaks = 0;
	...
	for (i = 0 ; i < fs_numServerReferencedPaks ; i++) {
		...
		if (FS_IsIdPak(fs_serverReferencedPakNames[i], "baseq3") || FS_IsIdPak(fs_serverReferencedPakNames[i], "missionpack")) {
			continue;
		}

		for (sp = fs_searchpaths ; sp ; sp = sp->next) {
			if (sp->pack && sp->pack->checksum == fs_serverReferencedPaks[i]) {
				havepak = 1; // This is it!
				break;
			}
		}
...
static void FS_Startup(const char *gameName) {
	...
	fs_gamedirvar->modified = 0; // We just loaded, it's not modified
...
baseq3フォルダのproductid.txtの内容をチェックして、ファイルがなかったり、
内容が不正なものの場合、デモモードに移行する。
===================
*/
