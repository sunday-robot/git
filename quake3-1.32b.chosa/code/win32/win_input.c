...
void IN_JoyMove( void ) {
	...
	buttonstate_changed = joy.ji.dwButtons ^ joy.oldbuttonstate;
	for ( i=0 ; i < joy.jc.wNumButtons ; i++ ) {
		...
		// on  on  -> no event
		int button_bit = 1 << i;
		if ((buttonstate_changed & button_bit) == 0)
			continue;
		Sys_QueEvent(g_wv.sysMsgTime, SE_KEY, K_JOY1 + i, ((buttonstate & button_bit) != 0) ? qtrue : qfalse, 0, NULL);
	}
	...
	for (i = 0; i < joy.jc.wNumAxes && i < 4 ; i++) {
		...
		fAxisValue = JoyToF( (&joy.ji.dwXpos)[i] );	// dwXpos, dwYpos, dwZpos, dwRpos, dwUpos, dwVpos
