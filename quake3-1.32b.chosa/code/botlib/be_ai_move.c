...
#include "l_struct.h"
#include "aasfile.h"
#include "../botlib_include/botlib.h"
#include "../botlib_include/be_aas.h"
#include "be_aas_funcs.h"
#include "be_interface.h"

#include "../botlib_include/be_ea.h"
#include "../botlib_include/be_ai_goal.h"
#include "../botlib_include/be_ai_move.h"
...
int BotOnMover(vec3_t origin, int entnum, aas_reachability_t *reach)
{
	...
	for (i = 0; i < 2; i++)
	{
		if (origin[i] > modelorigin[i] + maxs[i] + 16) return 0;
		if (origin[i] < modelorigin[i] + mins[i] - 16) return 0;
	} //end for
	//
	VectorCopy(origin, org);
	org[2] += 24;
...
int MoverDown(aas_reachability_t *reach)
{
	...
	if (origin[2] + maxs[2] < reach->start[2]) return 1;
	return 0;
...
int BotValidTravel(vec3_t origin, aas_reachability_t *reach, int travelflags)
{
	...
	if (AAS_TravelFlagForType(reach->traveltype) & ~travelflags) return 0;
	//don't go into areas with bad travel types
	if (AAS_AreaContentsTravelFlags(reach->areanum) & ~travelflags) return 0;
	return 1;
...
int BotAvoidSpots(vec3_t origin, aas_reachability_t *reach, bot_avoidspot_t *avoidspots, int numavoidspots)
{
	...
	{
		case TRAVEL_WALK: checkbetween = 1; break;
		case TRAVEL_CROUCH: checkbetween = 1; break;
		case TRAVEL_BARRIERJUMP: checkbetween = 1; break;
		case TRAVEL_LADDER: checkbetween = 1; break;
		case TRAVEL_WALKOFFLEDGE: checkbetween = 0; break;
		case TRAVEL_JUMP: checkbetween = 0; break;
		case TRAVEL_SWIM: checkbetween = 1; break;
		case TRAVEL_WATERJUMP: checkbetween = 1; break;
		case TRAVEL_TELEPORT: checkbetween = 0; break;
		case TRAVEL_ELEVATOR: checkbetween = 0; break;
		case TRAVEL_GRAPPLEHOOK: checkbetween = 0; break;
		case TRAVEL_ROCKETJUMP: checkbetween = 0; break;
		case TRAVEL_BFGJUMP: checkbetween = 0; break;
		case TRAVEL_JUMPPAD: checkbetween = 0; break;
		case TRAVEL_FUNCBOB: checkbetween = 0; break;
		default: checkbetween = 1; break;
	} //end switch
...
int BotMovementViewTarget(int movestate, bot_goal_t *goal, int travelflags, float lookahead, vec3_t target)
{
	...
	if (!ms->lastreachnum || !goal) return 0;
	...
	{
		AAS_ReachabilityFromNum(reachnum, &reach);
		if (BotAddToTarget(end, reach.start, lookahead, &dist, target)) return 1;
		//never look beyond teleporters
		if ((reach.traveltype & TRAVELTYPE_MASK) == TRAVEL_TELEPORT) return 1;
		//never look beyond the weapon jump point
		if ((reach.traveltype & TRAVELTYPE_MASK) == TRAVEL_ROCKETJUMP) return 1;
		if ((reach.traveltype & TRAVELTYPE_MASK) == TRAVEL_BFGJUMP) return 1;
		...
		{
			if (BotAddToTarget(reach.start, reach.end, lookahead, &dist, target)) return 1;
		} //end if
...
int BotCheckBarrierJump(bot_movestate_t *ms, vec3_t dir, float speed)
{
	...
	if (trace.endpos[2] - ms->origin[2] < sv_maxstep->value) return 0;
	//
	hordir[0] = dir[0];
	hordir[1] = dir[1];
	hordir[2] = 0;
...
int BotWalkInDirection(bot_movestate_t *ms, vec3_t dir, float speed, int type)
{
	...
	if (ms->moveflags & MFL_ONGROUND)
	{
		...
		if (BotCheckBarrierJump(ms, dir, speed)) return 1;
		...
		origin[2] += 0.5;
		AAS_PredictClientMovement(&move, ms->entitynum, origin, presencetype, 1,
		...
		if (VectorLength(tmpdir) < speed * ms->thinktime * 0.5) return 0;
...
bot_moveresult_t BotTravel_Walk(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	//
	BotCheckBlocked(ms, hordir, 1, &result);
	//
	if (dist < 10)
	{
...
bot_moveresult_t BotTravel_Swim(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	VectorNormalize(dir);
	//
	BotCheckBlocked(ms, dir, 1, &result);
	//elemantary actions
...
bot_moveresult_t BotTravel_Elevator(bot_movestate_t *ms, aas_reachability_t *reach)
{
	...
	else
	{
		...
		if (!MoverDown(reach))
		{
			...
			VectorCopy(dir1, dir);
			//
			BotCheckBlocked(ms, dir, 0, &result);
			//
			if (dist > 60) dist = 60;
			speed = 360 - (360 - 6 * dist);
			//
