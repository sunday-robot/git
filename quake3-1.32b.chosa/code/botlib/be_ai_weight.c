...
int ReadValue(source_t *source, float *value)
{
	...
	if (!strcmp(token.string, "-"))
	{
		...
		if (!PC_ExpectTokenType(source, TT_NUMBER, 0, &token)) return 0;
	} //end if
...
int ReadFuzzyWeight(source_t *source, fuzzyseperator_t *fs)
{
	if (PC_CheckTokenString(source, "balance"))
	{
		...
		if (!ReadValue(source, &fs->weight)) return 0;
		if (!PC_ExpectTokenString(source, ",")) return 0;
		if (!ReadValue(source, &fs->minweight)) return 0;
		if (!PC_ExpectTokenString(source, ",")) return 0;
		if (!ReadValue(source, &fs->maxweight)) return 0;
		if (!PC_ExpectTokenString(source, ")")) return 0;
	} //end if
	else
	{
...
{
	if (!WriteIndent(fp, indent)) return 0;
	...
	do
	{
		...
		if (fs->next)
		{
			if (fprintf(fp, "case %d:", fs->value) < 0) return 0;
		} //end if
		else
		{
			if (fprintf(fp, "default:") < 0) return 0;
		} //end else
		if (fs->child)
		{
			if (fprintf(fp, "\n") < 0) return 0;
			if (!WriteIndent(fp, indent)) return 0;
			if (fprintf(fp, "{\n") < 0) return 0;
			if (!WriteFuzzySeperators_r(fp, fs->child, indent + 1)) return 0;
			if (!WriteIndent(fp, indent)) return 0;
			if (fs->next)
			{
		...
		{
			if (!WriteFuzzyWeight(fp, fs)) return 0;
		} //end else
		fs = fs->next;
	} while(fs);
	indent--;
...
qboolean WriteWeightConfig(char *filename, weightconfig_t *config)
{
	...
	for (i = 0; i < config->numweights; i++)
	{
		...
		{
			if (!WriteFuzzySeperators_r(fp, ifw->firstseperator, 1)) return 0;
		} //end if
		else
		{
			if (!WriteIndent(fp, 1)) return 0;
			if (!WriteFuzzyWeight(fp, ifw->firstseperator)) return 0;
		} //end else
