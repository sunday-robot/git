										...
										token_t **firsttoken, token_t **lasttoken)
{
	...
	if (define->numparms)
	{
		if (!PC_ReadDefineParms(source, define, parms, MAX_DEFINEPARMS)) return 0;
#ifdef DEBUG_EVAL
...
int PC_ExpandDefineIntoSource(source_t *source, token_t *deftoken, define_t *define)
{
	...
	if (!PC_ExpandDefine(source, deftoken, define, &firsttoken, &lasttoken)) return 0;
...
int PC_ReadLine(source_t *source, token_t *token)
{
	...
	do
	{
		if (!PC_ReadSourceToken(source, token)) return 0;
...
int PC_Directive_define(source_t *source)
{
	...
	if (define)
	{
		...
		PC_UnreadSourceToken(source, &token);
		if (!PC_Directive_undef(source)) return 0;
	...
	if (!PC_ReadLine(source, &token)) return 1;
	...
	do
	{
		...
		if (token.type == TT_NAME)
		{
			...
			else
			{
				...
				if (!PC_ExpandDefineIntoSource(source, &token, define)) return 0;
			} //end else
		} //end if
...
int PC_Directive_elif(source_t *source)
{
	...
	if (!PC_Evaluate(source, &value, NULL, 1)) return 0;
	skip = (value == 0);
...
int PC_Directive_evalfloat(source_t *source)
{
	...
	if (!PC_Evaluate(source, NULL, &value, 0)) return 0;
	token.line = source->scriptstack->line;
	token.whitespace_p = source->scriptstack->script_p;
	token.endwhitespace_p = source->scriptstack->script_p;
	token.linescrossed = 0;
...
int PC_DollarDirective_evalint(source_t *source)
{
	...
	if (!PC_DollarEvaluate(source, &value, NULL, 1)) return 0;
	//
	token.line = source->scriptstack->line;
	token.whitespace_p = source->scriptstack->script_p;
	token.endwhitespace_p = source->scriptstack->script_p;
...
int PC_DollarDirective_evalfloat(source_t *source)
{
	...
	if (!PC_DollarEvaluate(source, NULL, &value, 0)) return 0;
	token.line = source->scriptstack->line;
	token.whitespace_p = source->scriptstack->script_p;
	token.endwhitespace_p = source->scriptstack->script_p;
	token.linescrossed = 0;
...
int QuakeCMacro(source_t *source)
{
	...
	if (!PC_ReadSourceToken(source, &token)) return 1;
	if (token.type != TT_NAME)
	{
		PC_UnreadSourceToken(source, &token);
		return 1;
	...
	while(1)
	{
		...
		if (token->type == TT_PUNCTUATION && *token->string == '#')
		{
...
#endif //QUAKC
			{
				...
				if (!PC_ReadDirective(source)) return 0;
				continue;
			} //end if
		} //end if
		if (token->type == TT_PUNCTUATION && *token->string == '$')
		{
			...
			{
				//read the precompiler directive
				if (!PC_ReadDollarDirective(source)) return 0;
				continue;
			} //end if
		} //end if
...
int PC_CheckTokenString(source_t *source, char *string)
{
	...
	if (!PC_ReadToken(source, &tok)) return 0;
