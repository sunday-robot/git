...
int AAS_BestReachableFromJumpPadArea(vec3_t origin, vec3_t mins, vec3_t maxs)
{
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		area2num = 0;
		AAS_ClientMovementHitBBox(&move, -1, areastart, PRESENCE_NORMAL, 0,
...
int AAS_BestReachableArea(vec3_t origin, vec3_t mins, vec3_t maxs, vec3_t goalorigin)
{
	...
	if (areanum)
	{
		...
		else
		{
			...
			//point will return trace.startsolid 1
...
//===========================================================================
// returns 1 if the area has reachability links
//
// Parameter:				-
// Returns:					-
// Changes Globals:		-
...
{
	if (!(aasworld.areasettings[areanum].presencetype & PRESENCE_NORMAL)) return 1;
	else return 0;
} //end of the function AAS_AreaCrouch
//===========================================================================
// returns 1 if it is possible to swim in the area
//
// Parameter:				-
// Returns:					-
// Changes Globals:		-
...
{
	if (aasworld.areasettings[areanum].areaflags & AREA_LIQUID) return 1;
	else return 0;
} //end of the function AAS_AreaSwim
//===========================================================================
// returns 1 if the area contains a liquid
//
// Parameter:				-
// Returns:					-
// Changes Globals:		-
...
//===========================================================================
// returns 1 if the area contains ground faces
//
// Parameter:				-
// Returns:					-
// Changes Globals:		-
...
qboolean AAS_ReachabilityExists(int area1num, int area2num)
{
	...
	{
		if (r->areanum == area2num) return 1;
	} //end for
	return 0;
...
int AAS_NearbySolidOrGap(vec3_t start, vec3_t end)
{
	...
	if (areanum)
	{
		if (!AAS_AreaSwim(areanum) && !AAS_AreaGrounded(areanum)) return 1;
	} //end if
	return 0;
...
int AAS_Reachability_Swim(int area1num, int area2num)
{
	...
	if (!AAS_AreaSwim(area1num) || !AAS_AreaSwim(area2num)) return 0;
	//if the second area is crouch only
	if (!(aasworld.areasettings[area2num].presencetype & PRESENCE_NORMAL)) return 0;
...
int AAS_Reachability_EqualFloorHeight(int area1num, int area2num)
{
	...
	if (area2->mins[2] > area1->maxs[2]) return 0;
	//
...
//		Log_Write("shortest distance between %d and %d is %f\r\n", area1num, area2num, bestdist);
		...
		for (i = 0; i < 3; i++)
		{
			...
			//
			AAS_PredictClientMovement(&move, -1, beststart, PRESENCE_NORMAL, 1,
...
//#endif REACH_DEBUG
		...
		if (!AAS_AreaTeleporter(area2num) && !AAS_AreaJumpPad(area2num))
		{
			...
			else*/
			{
				...
				VectorClear(cmdmove);
				AAS_PredictClientMovement(&move, -1, destorigin, PRESENCE_NORMAL, 0,
										velocity, cmdmove, 0, 30, 0.1f,
										SE_HITGROUND|SE_ENTERWATER|SE_ENTERSLIME|
										SE_ENTERLAVA|SE_HITGROUNDDAMAGE|SE_TOUCHJUMPPAD|SE_TOUCHTELEPORTER, 0, 0); //1);
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		*/
		...
		for (i = 0; i < 2; i++)
		{
			...
			if (i == 0)
			{
				firststartreach = AAS_FindFaceReachabilities(start_edgeverts, 4, &start_plane, 1);
				firstendreach = AAS_FindFaceReachabilities(end_edgeverts, 4, &end_plane, 0);
			} //end if
			else
			{
				firststartreach = AAS_FindFaceReachabilities(end_edgeverts, 4, &end_plane, 1);
				firstendreach = AAS_FindFaceReachabilities(start_edgeverts, 4, &start_plane, 0);
			} //end else
			//
...
//		VectorScale(velocity, speed, velocity);
        ...
        */
		...
		if (velocity[0] || velocity[1])
		{
			...
			for (i = 0; i < 20; i++)
			{
				AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL, 0,
										velocity, cmdmove, 0, 30, 0.1f,
										SE_HITGROUND|SE_ENTERWATER|SE_ENTERSLIME|
		...
		for (n = 0; n < 1/*2*/; n++)
		{
			...
			if (ret && speed < 300)
			{
				...
				//if (hordist < 1.6 * (facecenter[2] - areastart[2]))
				{
					...
					*/
					//
					AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL, 1,
												velocity, cmdmove, 30, 30, 0.1f,
												SE_ENTERWATER|SE_ENTERSLIME|
												SE_ENTERLAVA|SE_HITGROUNDDAMAGE|
...
int AAS_ContinueInitReachability(float time)
{
	...
	if (aasworld.numreachabilityareas >= aasworld.numareas + 2) return 0;
