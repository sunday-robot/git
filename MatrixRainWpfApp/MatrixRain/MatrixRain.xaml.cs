...
namespace MatrixRain
{
    ...
    public partial class MatrixRain : UserControl
    {
        ...
        public MatrixRain()
        {
            ...
            _DispatcherTimer.Elapsed += _OnDispatcherTimerElapsed;
		 ...
		 * </summary>
		 */
        private readonly Random _CryptoRandom = new Random();
        ...
        {
            var w = _MyCanvas.ActualWidth;
            var h = _MyCanvas.ActualHeight;

            _CanvasRect = new Rect(0, 0, w, h);

            _RenderTargetBitmap = new RenderTargetBitmap((int)w, (int)h, 96, 96, PixelFormats.Pbgra32);
            _WriteableBitmap = new WriteableBitmap(_RenderTargetBitmap);
            _MyImage.Source = _WriteableBitmap;
            _MyCanvas.Measure(new Size(w, h));
            _MyCanvas.Arrange(new Rect(new Size(w, h)));

            SetParameter(framePerSecond: _FramePerSecond, fontSize: (int)_RenderingEmSize);
            _Drops = new int[(int)(w / _LetterAdvanceWidth)];
            for (var x = 0; x < _Drops.Length; x++)
                _Drops[x] = 1;
        }
		 ...
		 * </summary>
		 */
        private void _OnDispatcherTimerElapsed(object sender, EventArgs e)
        {
            if (!Dispatcher.CheckAccess())
            {
                //synchronize on main thread
                System.Timers.ElapsedEventHandler dt = _OnDispatcherTimerElapsed;
